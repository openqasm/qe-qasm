...
public:
  ...
  ASTFunctionCallNode(const ASTIdentifierNode* Id,
                      const ASTFunctionDefinitionNode* FD,
                      const ASTExpressionList* EL,
                      const ASTIdentifierList* QL)
  : ASTExpressionNode(Id, ASTTypeFunctionCallExpression),
  CId(FD->GetIdentifier()), FDN(FD), CEL(*EL), QIL(*QL),
  CallType(ASTTypeFunctionCallExpression), STEMap() { }

  ASTFunctionCallNode(const ASTIdentifierNode* Id,
                      const ASTDefcalNode* DD,
                      const ASTExpressionList* EL)
  ...
  virtual ASTType GetFunctionCallType() const {
    return CallType;
  }
  ...
  virtual const ASTIdentifierNode* GetCallIdentifier() const {
    return CId;
  }

  virtual const std::string& GetCallName() const {
    return CId->GetName();
  }

  virtual unsigned GetNumArguments() const {
    return QIL.Size() + CEL.Size();
  }
  ...
  virtual const ASTExpressionList& GetExpressionList() const {
    return CEL;
  }
  ...
  virtual const std::map<unsigned, ASTSymbolTableEntry*>& GetSymbolMap() const {
    return STEMap;
  }
...
public:
  ...
  virtual const ASTFunctionCallNode*
  ...
  static ASTFunctionCallStatementNode* StatementError(const ASTIdentifierNode* Id,
                                                      const std::string& ERM) {
...
public:
  static ASTFunctionCallValidator& Instance() {
    return FCV;
  }
  ...
  void ValidateArgument(const ASTSymbolTableEntry* XSTE,
                        unsigned XI,
                        const std::string& AN,
                        const std::string& FN);
};
