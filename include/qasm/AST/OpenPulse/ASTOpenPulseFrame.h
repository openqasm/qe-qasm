...
public:
  ASTOpenPulseFrameNode(const ASTIdentifierNode* Id)
  : ASTExpressionNode(Id, ASTTypeOpenPulseFrame),
  PRT(nullptr), FRQ(nullptr), PHS(nullptr), DUR(nullptr),
  EXT(false) { }

  ASTOpenPulseFrameNode(const ASTIdentifierNode* Id,
                        const ASTOpenPulsePortNode* PN,
                        const ASTMPDecimalNode* FR,
                        const ASTAngleNode* PH,
                        bool Extern = false)
  : ASTExpressionNode(Id, ASTTypeOpenPulseFrame),
  PRT(PN), FRQ(FR), PHS(PH), DUR(nullptr), EXT(Extern) { }
  ...
  ASTOpenPulseFrameNode(const ASTIdentifierNode* Id,
                        ...
                        bool Extern = false)
  : ASTExpressionNode(Id, ASTTypeOpenPulseFrame),
  PRT(PN), FRQ(FR), PHS(PH), DUR(DN), EXT(Extern) { }
  ...
  const ASTOpenPulsePortNode* GetPort() const {
    return PRT;
  }

  const ASTMPDecimalNode* GetFrequency() const {
    return FRQ;
  }

  const ASTAngleNode* GetPhase() const {
    return PHS;
  }

  const ASTDurationNode* GetDuration() const {
    return DUR;
  }

  ASTDurationNode* GetDuration() {
    return const_cast<ASTDurationNode*>(DUR);
  }

  void SetPhase(const ASTAngleNode* PH) {
    PHS = PH;
  }

  void SetFrequency(const ASTMPDecimalNode* MPD) {
    FRQ = MPD;
  }

  void SetDuration(const ASTDurationNode* DN) {
    DUR = DN;
  }

  void SetExtern(bool EV) {
    EXT = EV;
  }
...
public:
  static ASTOpenPulseFrameNodeResolver& Instance() {
    return FRN;
  }
