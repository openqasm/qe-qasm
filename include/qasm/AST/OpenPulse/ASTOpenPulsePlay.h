...
public:
  ...
  : ASTExpressionNode(Id, ASTTypeOpenPulsePlay), WN(W),
  FC(nullptr), FN(F) {
  ...
  ASTOpenPulsePlayNode(const ASTIdentifierNode* Id,
                       ...
                       const ASTOpenPulseFrameNode* F)
  : ASTExpressionNode(Id, ASTTypeOpenPulsePlay), WN(nullptr),
  FC(C), FN(F) {
    assert(F && "Invalid ASTOpenPulseFrameNode argument!");
  }
  ...
  FC(nullptr), FN(F) {
    assert(F && "Invalid ASTOpenPulseFrameNode argument!");
    WN = new ASTOpenPulseWaveformNode(&ASTIdentifierNode::Waveform,
                                      CXL);
  ...
  virtual bool HasWaveform() const {
    return WN != nullptr;
  }

  const ASTOpenPulseWaveformNode* GetWaveform() {
    return WN;
  }

  const ASTOpenPulseWaveformNode* GetWaveform() const {
    return WN;
  }

  const ASTFunctionCallNode* GetFunctionCall() {
    return FC;
  }

  const ASTFunctionCallNode* GetFunctionCall() const {
    return FC;
  }

  const ASTOpenPulseFrameNode* GetFrame() {
    return FN;
  }

  const ASTOpenPulseFrameNode* GetFrame() const {
    return FN;
  }
