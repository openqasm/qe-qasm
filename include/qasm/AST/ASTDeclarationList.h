...
public:
  ...
  virtual void Append(ASTDeclarationNode* DN) {
    this->push(DN);
  }
  ...
  // Implemented in ASTDeclarationBuilder.cpp.
  virtual bool TransferSymbols(std::map<std::string,
                               const ASTSymbolTableEntry*>& M) const;
  ...
  ASTDeclarationNode* front() {
    return List.front();
  }

  const ASTDeclarationNode* front() const {
    return List.front();
  }

  ASTDeclarationNode* back() {
    return List.back();
  }

  const ASTDeclarationNode* back() const {
    return List.back();
  }
