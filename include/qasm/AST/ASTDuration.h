...
public:
  ASTDurationOfNode(const ASTIdentifierNode* Id,
                    const ASTIdentifierNode* TId)
  : ASTStatementNode(Id), Target(TId), Args(), Qubits(),
  QOP(nullptr) { }

  ASTDurationOfNode(const ASTIdentifierNode* Id,
                    const ASTIdentifierNode* TId,
                    const ASTIdentifierList& QIL)
  : ASTStatementNode(Id), Target(TId), Args(), Qubits(QIL),
  QOP(nullptr) { }
  ...
  QOP(nullptr) {
    ...
    assert(Args.size() == AIL.Size() &&
           "Inconsistent Arguments size!");
  }
  ...
  virtual bool HasOpTarget() const {
    return QOP != nullptr;
  }
  ...
  const ASTGateQOpNode* GetOpTarget() const {
    return QOP;
  }

  ASTGateQOpNode* GetOpTarget() {
    return const_cast<ASTGateQOpNode*>(QOP);
  }

  const ASTIdentifierList& GetQubits() const {
    return Qubits;
  }

  const std::vector<ASTOperandNode*>& GetArgs() const {
    return Args;
  }
  ...
  virtual void print() const override {
    ...
    if (!Args.empty()) {
      for (std::vector<ASTOperandNode*>::const_iterator I = Args.begin();
           I != Args.end(); ++I) {
        if ((*I)->IsExpression())
          std::cout << "<Argument>" << (*I)->GetExpression()->GetPolymorphicName()
            << "</Argument>" << std::endl;
        else
          std::cout << "<Argument>" << (*I)->GetIdentifier()->GetPolymorphicName()
            << "</Argument>" << std::endl;
      }
    }
...
public:
  ...
  : ASTExpressionNode(Id, ASTTypeDuration),
  Duration(0UL), Units(LengthOfDependent), DTy(LON->GetASTType()),
  LO(LON), BOP(nullptr) { }
  ...
  virtual ASTType GetDurationType() const {
    return DTy;
  }

  virtual bool IsDurationOf() const {
    return DTy == ASTTypeDurationOf;
  }

  virtual bool IsDuration() const {
    return DTy == ASTTypeDuration;
  }

  virtual std::string AsString() const;

  virtual const ASTDurationOfNode* GetOperand() const {
    return LO;
  }
  ...
  virtual bool HasFunctionCall() const {
    return EX != nullptr;
  }
  ...
  virtual const ASTExpressionNode* GetFunctionCall() const {
    return EX;
  }
  ...
  static ASTDurationNode* ExpressionError(const std::string& ERM) {
    return new ASTDurationNode(ASTIdentifierNode::Duration.Clone(),
                               ERM, ASTTypeExpressionError);
  }
