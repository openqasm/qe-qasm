...
public:
  ...
  ASTLocation(const ASTLocation &RHS) : LineNo(RHS.LineNo), ColNo(RHS.ColNo) {}

  ASTLocation(uint32_t L, uint32_t C) : LineNo(L), ColNo(C) {}
  ...
  inline uint32_t GetLineNo() const { return LineNo; }

  inline uint32_t GetColNo() const { return ColNo; }
};

enum ASTTokenType : unsigned { TTUndefined = 0, TTInt, TTString };
...
public:
  ASTToken(int32_t TT, uint32_t XI) : TI(TT), TS(), TTy(TTInt), Loc(), IX(XI) {}
  ...
  ASTToken(const ASTToken &RHS)
      : TI(RHS.TI), TS(RHS.TS), TTy(RHS.TTy), Loc(RHS.Loc), IX(RHS.IX) {}
  ...
  void SetTokenType(ASTTokenType T) { TTy = T; }
  ...
  void SetLocation(const ASTLocation &L) { Loc = L; }
  ...
  ASTTokenType GetTokenType() const { return TTy; }

  int64_t GetInt() const { return TI; }

  const std::string &GetString() const { return TS; }

  const char *GetConstChar() const { return TS.data(); }

  const std::string *GetStringAddress() const { return &TS; }

  uint32_t GetIndex() const { return IX; }

  const ASTLocation &GetLocation() const { return Loc; }
};
...
public:
  ...
  virtual const ASTLocation &GetLocation() const { return Loc; }

  virtual uint32_t GetLineNo() const { return Loc.LineNo; }

  virtual uint32_t GetColNo() const { return Loc.ColNo; }
