...
public:
  ASTArgumentNode(const ASTIdentifierNode *Id, const std::any &Val, ASTType Ty)
  ...
  virtual ASTType GetASTType() const override { return ASTTypeArgument; }

  virtual bool IsExpression() const override { return Expr; }

  virtual bool IsConstExpr() const { return ConstExpr; }

  virtual bool IsValue() const { return Value.second != ASTTypeUndefined; }

  virtual bool IsIdentifier() const override { return Ident.first; }

  virtual bool IsIdentifierRef() const { return Ident.second; }
    ...
    if (IsIdentifier())
      ...
      return IsConstExpr()
                 ? std::any_cast<const ASTExpressionNode *>(Value.first)
                       ->GetIdentifier()
                 : std::any_cast<ASTExpressionNode *>(Value.first)
                       ->GetIdentifier();
  ...
  virtual const std::string &GetName() const override {
    if (IsIdentifier())
      return Ident.first ? Ident.first->GetName() : Ident.second->GetName();
    else if (IsExpression()) {
      try {
        ...
        return ID ? ID->GetName() : ASTIdentifierNode::Null.GetName();
  ...
  virtual std::any &GetValue() { return Value.first; }

  virtual const std::any &GetValue() const { return Value.first; }

  virtual ASTType GetValueType() const { return Value.second; }
  ...
  virtual void print() const override {
    ...
    std::cout << "<IsValue>" << std::boolalpha << IsValue() << "</IsValue>"
              << std::endl;
...
public:
  ...
  ASTArgumentNodeList(const ASTArgumentNodeList &RHS) : Graph(RHS.Graph) {}

  ASTArgumentNodeList(const ASTExpressionList *EL) : Graph() { *this = EL; }
  ...
  virtual bool Empty() const { return Graph.empty(); }
