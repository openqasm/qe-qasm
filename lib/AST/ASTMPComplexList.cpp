...
void
ASTMPComplexList::ConstructFromExpressionList(const ASTExpressionList& EL) {
  if (EL.Empty())
    return;
    ...
    if (EN) {
      ...
      case ASTTypeMPComplex: {
        if (const ASTMPComplexNode* MPC =
          ...
          List.push_back(new ASTMPComplexRep(const_cast<ASTMPComplexNode*>(MPC)));
        }
      }
        break;
      case ASTTypeBinaryOp: {
        if (const ASTBinaryOpNode* BOP =
          ...
          List.push_back(new ASTMPComplexRep(const_cast<ASTBinaryOpNode*>(BOP),
                                             MPC));
        }
      }
        break;
      ...
      case ASTTypeIdentifier: {
        ...
        while (!STE && NumBits < 512) {
          STE = ASTSymbolTable::Instance().Lookup(Id, NumBits, ASTTypeMPComplex);
          if (STE) {
...
ASTMPComplexRep::MangleIdentifier(const ASTIdentifierNode* Id) const {
  ...
  case ASTTypeMPInteger: {
    if (ASTMPIntegerNode* MPI =
        Id->GetSymbolTableEntry()->GetValue()->GetValue<ASTMPIntegerNode*>()) {
      MPI->Mangle();
  ...
  case ASTTypeMPDecimal: {
    if (ASTMPDecimalNode* MPD =
        Id->GetSymbolTableEntry()->GetValue()->GetValue<ASTMPDecimalNode*>()) {
      MPD->Mangle();
    ...
    break;
  case ASTTypeMPComplex: {
    if (ASTMPComplexNode* MMPC =
        Id->GetSymbolTableEntry()->GetValue()->GetValue<ASTMPComplexNode*>()) {
      MMPC->Mangle();
...
std::string ASTMPComplexRep::Mangle() {
  ...
  if (MPC) {
    MPC->Mangle();
    M.StringValue(ASTStringUtils::Instance().SanitizeMangled(MPC->GetMangledName()));
  }
