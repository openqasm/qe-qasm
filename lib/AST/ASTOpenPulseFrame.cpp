...
ASTOpenPulseFrameNodeResolver::ResolveAngle(ASTExpressionNode* E) {
  ...
  case ASTTypeDouble: {
    ...
    AN = new ASTAngleNode(&ASTIdentifierNode::Angle,
                          DN, ASTAngleTypeGeneric, 64);
  }
    break;
  case ASTTypeFloat: {
  ...
  case ASTTypeIdentifier: {
    ASTIdentifierNode* AId = const_cast<ASTIdentifierNode*>(E->GetIdentifier());
    if (!AId) {
      std::stringstream M;
      M << "Invalid second element for the frame initialization "
        << "ExpressionList.";
    ...
    for (std::vector<std::vector<ASTSymbolTableEntry*>::iterator>::iterator EI =
         EV.begin(); EI != EV.end(); ++EI)
      STV.erase(*EI);
    ...
    for (std::vector<ASTSymbolTableEntry*>::iterator SI = STV.begin();
      ...
      case ASTTypeFloat: {
        ...
        assert(FN && "Could not obtain a valid ASTFloatNode from the SymbolTable!");
        AN = new ASTAngleNode(&ASTIdentifierNode::Angle,
                              FN, ASTAngleTypeGeneric, 32);
        break;
      }
      ...
      case ASTTypeDouble: {
        ...
        assert(DN && "Could not obtain a valid ASTDoubleNode from the SymbolTable!");
        AN = new ASTAngleNode(&ASTIdentifierNode::Angle,
                              DN, ASTAngleTypeGeneric, 64);
        break;
      }
        break;
      case ASTTypeMPDecimal: {
        ASTMPDecimalNode* MPD = ASTE->GetValue()->GetValue<ASTMPDecimalNode*>();
        assert(MPD && "Could not obtain a valid ASTMPDecimalNode from the SymbolTable!");
      ...
      case ASTTypeAngle: {
        ...
        assert(AN && "Could not obtain a valid ASTAngleNode from the SymbolTable!");
        break;
      }
        break;
      default: {
  ...
  case ASTTypeBinaryOp: {
    ...
    AN = new ASTAngleNode(&ASTIdentifierNode::Angle, BOP,
  ...
  case ASTTypeUnaryOp: {
    ...
    AN = new ASTAngleNode(&ASTIdentifierNode::Angle, UOP,
                          ASTAngleTypeGeneric, 32);
  }
    break;
  default: {
...
ASTOpenPulseFrameNodeResolver::ResolveFrequency(ASTExpressionNode* E) {
  ...
  case ASTTypeMPDecimal: {
    ...
    break;
  case ASTTypeIdentifier: {
    ASTIdentifierNode* DId = const_cast<ASTIdentifierNode*>(E->GetIdentifier());
    if (!DId) {
      std::stringstream M;
      M << "Invalid second element for the frame initialization "
        << "ExpressionList.";
    ...
    for (std::vector<ASTSymbolTableEntry*>::iterator SI = STV.begin();
      ...
      case ASTTypeInt: {
        ...
        assert(I && "Could not obtain a valid ASTIntNode from the SymbolTable!");
      ...
      case ASTTypeFloat: {
        ...
        assert(F && "Could not obtain a valid ASTFloatNode from the SymbolTable!");
        MPD = new ASTMPDecimalNode(&ASTIdentifierNode::MPDec, 32, F->GetValue());
      }
        break;
      case ASTTypeDouble: {
        ASTDoubleNode* D = DSTE->GetValue()->GetValue<ASTDoubleNode*>();
        assert(D && "Could not obtain a valid ASTDoubleNode from the SymbolTable!");
        MPD = new ASTMPDecimalNode(&ASTIdentifierNode::MPDec, 64, D->GetValue());
      }
        break;
      case ASTTypeMPInteger: {
        ASTMPIntegerNode* MPI = DSTE->GetValue()->GetValue<ASTMPIntegerNode*>();
  ...
  case ASTTypeIdentifierRef: {
    ...
    for (std::vector<ASTSymbolTableEntry*>::iterator SI = STV.begin();
      ...
      case ASTTypeMPIntegerArray: {
        ...
        unsigned Bits = MPI->GetBits();
        Bits += Bits / 4;
        MPD = new ASTMPDecimalNode(&ASTIdentifierNode::MPDec, Bits,
                                   MPI);
      }
        break;
      case ASTTypeIdentifier: {
...
ASTOpenPulseFrameNodeResolver::ResolveFrequency(const ASTIdentifierNode* Id) {
  ...
  if (!STE) {
    STE = ASTSymbolTable::Instance().Lookup(Id, Id->GetBits(), Id->GetValueType());
    if (!STE) {
      std::stringstream M;
...
ASTMPDecimalNode*
ASTOpenPulseFrameNodeResolver::ResolveFrequency(const ASTIdentifierRefNode* IdR) {
...
ASTOpenPulseFrameNodeResolver::ResolveDuration(ASTExpressionNode* E0,
  ...
  if (!I0) {
    ...
    QasmDiagnosticEmitter::Instance().EmitDiagnostic(
      DIAGLineCounter::Instance().GetLocation(E0), M.str(),
                                                   DiagLevel::Error);
    return nullptr;
  }
  ...
  if (!I1) {
    ...
    QasmDiagnosticEmitter::Instance().EmitDiagnostic(
      DIAGLineCounter::Instance().GetLocation(E1), M.str(),
                                                   DiagLevel::Error);
    return nullptr;
  }
...
void ASTOpenPulseFrameNode::Mangle() {
  ...
  if (PRT) {
    ...
    M.StringValue(ASTStringUtils::Instance().SanitizeMangled(PRT->GetMangledName()));
  }
  ...
  if (DUR) {
    ...
    M.StringValue(ASTStringUtils::Instance().SanitizeMangled(DUR->GetMangledName()));
  }
