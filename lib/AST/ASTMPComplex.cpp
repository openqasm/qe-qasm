...
void
ASTMPComplexNode::Evaluate(const ASTMPDecimalNode* R,
  ...
  switch (OT) {
  case ASTOpTypeAdd:
    SetBias(mpc_set_fr_fr(MPValue, R->GetMPValue(), I->GetMPValue(),
                          MPC_RNDND));
    NE = false;
    return;
    break;
...
void
ASTMPComplexNode::Evaluate(const ASTMPIntegerNode* R,
  ...
  switch (OT) {
  case ASTOpTypeAdd:
    SetBias(mpc_set_z_z(MPValue, R->GetMPValue(),
                    I->GetMPValue(), MPC_RNDND));
    NE = false;
    return;
    break;
  case ASTOpTypeSub: {
...
void ASTMPComplexNode::Evaluate(const ASTMPComplexNode* L,
  ...
  switch (OT) {
  case ASTOpTypeAdd:
    SetBias(mpc_add(MPValue, L->GetMPValue(), R->GetMPValue(),
                    MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSub:
    SetBias(mpc_sub(MPValue, L->GetMPValue(), R->GetMPValue(),
                    MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeMul:
    SetBias(mpc_mul(MPValue, L->GetMPValue(), R->GetMPValue(),
                    MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeDiv:
    SetBias(mpc_div(MPValue, L->GetMPValue(), R->GetMPValue(),
                    MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypePow:
    SetBias(mpc_pow(MPValue, L->GetMPValue(), R->GetMPValue(),
                    MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSqrt:
  ...
  default: {
    mpc_set_nan(MPValue);
    std::stringstream M;
    M << "Invalid operands to binary expression '"
      << PrintOpTypeEnum(OT) << "'.";
    QasmDiagnosticEmitter::Instance().EmitDiagnostic(
      DIAGLineCounter::Instance().GetLocation(), M.str(), DiagLevel::Error);
  }
...
void ASTMPComplexNode::Evaluate(const ASTMPComplexNode* L,
  ...
  switch (OT) {
  case ASTOpTypeAdd:
    SetBias(mpc_add_fr(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSub:
    SetBias(mpc_sub_fr(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeMul:
    SetBias(mpc_mul_fr(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeDiv:
    SetBias(mpc_div_fr(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypePow:
    SetBias(mpc_pow_fr(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSqrt:
...
void ASTMPComplexNode::Evaluate(const ASTMPComplexNode* L,
  ...
  case ASTOpTypeDiv:
    ...
    break;
  case ASTOpTypePow:
    SetBias(mpc_pow_fr(MPValue, L->GetMPValue(), MPVR,
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSqrt:
...
void ASTMPComplexNode::Evaluate(const ASTMPDecimalNode* L,
  ...
  switch (OT) {
  case ASTOpTypeAdd:
    SetBias(mpc_add_fr(MPValue, R->GetMPValue(), L->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeSub:
    SetBias(mpc_fr_sub(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeMul:
    SetBias(mpc_mul_fr(MPValue, R->GetMPValue(), L->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypeDiv:
    SetBias(mpc_fr_div(MPValue, L->GetMPValue(), R->GetMPValue(),
                       MPC_RNDND));
    NE = false;
    break;
  case ASTOpTypePow: {
...
void ASTMPComplexNode::Evaluate(const ASTIntNode* L,
                                const ASTMPComplexNode* R,
                                ASTOpType OT) {
  RBits = L->GetBits();
...
void
ASTMPComplexNode::Evaluate(const ASTFloatNode* R,
                           const ASTMPDecimalNode* I,
                           ASTOpType OT,
                           unsigned NumBits) {
...
void
ASTMPComplexNode::Evaluate(const ASTDoubleNode* R,
                           const ASTMPDecimalNode* I,
                           ASTOpType OT,
                           unsigned NumBits) {
  RBits = std::max(R->GetBits(), I->GetBits());
...
void
ASTMPComplexNode::Evaluate(const ASTIntNode* R,
                           const ASTMPDecimalNode* I,
                           ASTOpType OT,
                           unsigned NumBits) {
  RBits = std::max(R->GetBits(), I->GetBits());
...
void ASTMPComplexNode::Evaluate(const ASTIntNode* R,
                                const ASTDoubleNode* I,
                                ASTOpType OT,
                                unsigned NumBits) {
  RBits = std::max(R->GetBits(), I->GetBits());
...
ASTMPComplexNode::Evaluate(const ASTFloatNode* R,
  ...
  ASTMPDecimalNode* MPDR =
    new ASTMPDecimalNode(&ASTIdentifierNode::MPDec, NumBits,
                         static_cast<double>(R->GetValue()));
  ASTMPDecimalNode* MPDI =
    new ASTMPDecimalNode(&ASTIdentifierNode::MPDec, NumBits, I->GetValue());
...
void
ASTMPComplexNode::Evaluate(const ASTIdentifierNode* RId,
  ...
  ASTSymbolTableEntry* ISTE =
    ASTSymbolTable::Instance().Lookup(IId, IId->GetBits(), IId->GetSymbolType());
  ...
  if (RSTE->GetValueType() == ISTE->GetValueType()) {
    ...
    if (RSTE->GetValueType() == ASTTypeMPDecimal &&
      ...
      M << "Impossible Complex value initialization from R="
        << PrintTypeEnum(RSTE->GetValueType()) << " I="
        << PrintTypeEnum(ISTE->GetValueType()) << ".";
      QasmDiagnosticEmitter::Instance().EmitDiagnostic(
        DIAGLineCounter::Instance().GetLocation(), M.str(), DiagLevel::Error);
      return;
    }
...
void ASTMPComplexNode::Evaluate(const ASTComplexExpressionNode* E,
  ...
  if (RTy == ITy) {
    switch (RTy) {
    case ASTTypeMPDecimal: {
      Evaluate(dynamic_cast<const ASTMPDecimalNode*>(
                                  E->GetBinaryOp()->GetLeft()),
               dynamic_cast<const ASTMPDecimalNode*>(
                                  E->GetBinaryOp()->GetRight()),
               E->GetBinaryOp()->GetOpType(), NumBits);
      return;
    }
      break;
    case ASTTypeMPInteger: {
      Evaluate(dynamic_cast<const ASTMPIntegerNode*>(
                                  E->GetBinaryOp()->GetLeft()),
               dynamic_cast<const ASTMPIntegerNode*>(
                                  E->GetBinaryOp()->GetRight()),
               E->GetBinaryOp()->GetOpType(), NumBits);
      return;
    }
    ...
    if (RTy == ASTTypeMPDecimal && ITy == ASTTypeMPInteger) {
      Evaluate(dynamic_cast<const ASTMPIntegerNode*>(
                                  E->GetBinaryOp()->GetLeft()),
               dynamic_cast<const ASTMPDecimalNode*>(
                                  E->GetBinaryOp()->GetRight()),
      ...
      Evaluate(dynamic_cast<const ASTMPIntegerNode*>(E->GetBinaryOp()->GetLeft()),
               RDV, E->GetBinaryOp()->GetOpType(), 128U);
      NE = true;
      ...
      Evaluate(dynamic_cast<const ASTMPDecimalNode*>(E->GetBinaryOp()->GetLeft()),
               RDV, E->GetBinaryOp()->GetOpType(), 128U);
      NE = true;
      ...
      case ASTTypeMPDecimal: {
        Evaluate(RSTE->GetValue()->GetValue<ASTMPDecimalNode*>(),
                 dynamic_cast<const ASTFloatNode*>(
                                    E->GetBinaryOp()->GetRight()),
                 E->GetBinaryOp()->GetOpType(), NumBits);
        return;
      }
      ...
      case ASTTypeMPDecimal: {
        Evaluate(RSTE->GetValue()->GetValue<ASTMPDecimalNode*>(),
                 dynamic_cast<const ASTDoubleNode*>(
                                    E->GetBinaryOp()->GetRight()),
                 E->GetBinaryOp()->GetOpType(), NumBits);
        return;
      }
      ...
      switch (ISTE->GetValueType()) {
      case ASTTypeMPDecimal: {
        Evaluate(dynamic_cast<const ASTFloatNode*>(
                                    E->GetBinaryOp()->GetLeft()),
                 ISTE->GetValue()->GetValue<ASTMPDecimalNode*>(),
                 E->GetBinaryOp()->GetOpType(), NumBits);
        return;
      ...
      switch (ISTE->GetValueType()) {
      case ASTTypeMPDecimal: {
        Evaluate(dynamic_cast<const ASTDoubleNode*>(
                                    E->GetBinaryOp()->GetLeft()),
                 ISTE->GetValue()->GetValue<ASTMPDecimalNode*>(),
                 E->GetBinaryOp()->GetOpType(), NumBits);
        return;
      ...
      switch (ISTE->GetValueType()) {
      case ASTTypeMPDecimal: {
        Evaluate(dynamic_cast<const ASTIntNode*>(
                                    E->GetBinaryOp()->GetLeft()),
                 ISTE->GetValue()->GetValue<ASTMPDecimalNode*>(),
                 E->GetBinaryOp()->GetOpType(), NumBits);
        return;
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
  ...
  Bits(NumBits), Precision(NumBits), DeclBits(NumBits), OpType(OT),
  MPValue(), Expr(nullptr), FC(nullptr), NE(true),
  RTy(R->GetASTType()), ITy(I->GetASTType()), RBits(R->GetBits()),
  IBits(I->GetBits()) {
  Id->SetBits(NumBits);
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
  ...
  : ASTExpressionNode(Id, ASTTypeMPComplex),
  Bits(NumBits), Precision(NumBits), DeclBits(NumBits), OpType(OT),
  Bias(Exact), MPValue(), Expr(nullptr), FC(nullptr), NE(true),
  RTy(R->GetASTType()), ITy(I->GetASTType()), RBits(R->GetBits()),
  IBits(I->GetBits()) {
  Id->SetBits(NumBits);
  mpc_init2(MPValue, NumBits);
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
                                   ...
                                   unsigned NumBits)
  : ASTExpressionNode(Id, ASTTypeMPComplex),
  Bits(NumBits), Precision(NumBits), DeclBits(NumBits), OpType(ASTOpTypeNone),
  Bias(Exact), MPValue(), Expr(nullptr), FC(nullptr), NE(true),
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
  ...
  if (mpc_strtoc(MPValue, REP, nullptr, 0, MPC_RNDND) == -1) {
    std::stringstream M;
    M << "Unable to parse the string representation of complex number: "
      << REP << ". The value will be set to NaN.";
    QasmDiagnosticEmitter::Instance().EmitDiagnostic(
      DIAGLineCounter::Instance().GetLocation(), M.str(), DiagLevel::Warning);
    mpc_set_nan(MPValue);
  }
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
  ...
  : ASTExpressionNode(Id, ASTTypeMPComplex),
  Bits(NumBits), Precision(NumBits), DeclBits(NumBits), OpType(ASTOpTypeNone),
  Bias(Exact), MPValue(), Expr(nullptr), FC(FN), NE(true),
  RTy(ASTTypeMPDecimal), ITy(ASTTypeImaginary), RBits(DefaultBits),
  IBits(DefaultBits) {
  Id->SetBits(NumBits);
  mpc_init2(MPValue, NumBits);
...
ASTMPComplexNode::ASTMPComplexNode(const ASTIdentifierNode* Id,
                                   ...
                                   unsigned NumBits)
  : ASTExpressionNode(Id, ASTTypeMPComplex),
  Bits(NumBits), Precision(MPCC.Precision), DeclBits(MPCC.DeclBits),
  OpType(MPCC.OpType), Bias(MPCC.Bias), MPValue(), Expr(MPCC.Expr),
  FC(MPCC.FC), NE(MPCC.NE), RTy(MPCC.RTy), ITy(MPCC.ITy), RBits(MPCC.RBits),
  IBits(MPCC.IBits) {
    mpc_init2(MPValue, NumBits);
...
void ASTMPComplexNode::print() const {
  ...
  std::cout << "<RealBits>" << std::dec << RBits << "</RealBits>"
    << std::endl;
  std::cout << "<ImaginaryBits>" << std::dec << IBits
    << "</ImaginaryBits>" << std::endl;
