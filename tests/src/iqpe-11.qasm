...
// 11. read calibrations and enforce full schedule

// Compiler pass inserts defcals and resolves timing:
//
// For the scheduling, let's say that y90p is 20 ns on 0, 30 ns on 1,
// and cr90p on (0,1) is 300 ns. Let's say phase is 0 ns and the entire
// measurement is 400 ns.
//
// Questions:
//
// - How do we deal with casting to different precisions? Here the phase
//   defcal takes 20-bit angles, but the gate definitions above use 32-bit
//   angles. The same question comes up with the openpulse pi keyword I suppose.
// - How are we going to do reset in this example? Measure and do a bit flip?
//   Do we insert this in a previous pass, or do we define this behavior
//   in a defcal?
